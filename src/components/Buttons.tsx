...
export const Button = ({ children, disabled, ...props }: ButtonProps) => {
  const [canBeAnimated, setCanBeAnimateds] = useState(
    props.squared ? true : false
  );
...
const StyledButton = styled(motion.button)<ButtonProps>`
  ...
  * {
  ...
  pointer-events: ${({ disabled: deactivated }) =>
    deactivated ? "none" : "auto"};

  &:focus-visible {
    box-shadow: 0 0 0 3px
      ${({ theme }) => tinycolor(theme.global.accent).darken(20).toString()};
  }
`;
