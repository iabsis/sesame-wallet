  ...
  const refreshSubscriptions = (jwtToken: string | String | null) => {
    ...
    if (jwtToken) {
      getSubscriptions(jwtToken, true)
        .then((subData) => {
          setOpenedSubscriptions(subData.data);
        })
        .catch((err) => {
...
=======
                <Button full onClick={() => goToPage("/choose-plan")}>
                  <i className="icon-pick-axe pr-2"></i> Subscribe to our cloud
                  mining service
>>>>>>> 8fedeececaeae1475ecf5b3804405c73b6e2e37b
                </Button>
              </FooterActions>
            )}
          </>
...
const WalletContainer = styled(IonPage)`
  display: flex;
  flex: 1;
  overflow: hidden;
